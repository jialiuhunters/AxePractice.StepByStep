...
namespace Manualfac.Sources
{
    class OpenGenericRegistrationSource : IRegistrationSource
    {
        ...
        public ComponentRegistration RegistrationFor(Service service)
        {
            ...
            if (!serviceWithType.ChangeType(serviceType.GetGenericTypeDefinition()).Equals(genericService))
            {
                return null;
            }

            return new ComponentRegistration(service, new ReflectiveActivator(implementorType.MakeGenericType(serviceType.GetGenericArguments())));
